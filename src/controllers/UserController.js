...
//         A unique user is defined by its email since every Google Account must have a unique email address.
// @route  PUT /users
// @access Public
const addOrUpdateUser = asyncHandler(async (req, res) => {
  const { email, name } = req.body;
  // Finds a user based off email and name and returns an object with only the id, email, name fields
